...
/**
 * <p>
 * NOTE: This is customized version of {@link SpringBeanProcessor}
 * with the following changes: 
 * </p><ul>
 *     <li>only requires {@link ResteasyProviderFactory} as a direct dependency</li>
 *     <li>no longer depends on {@link ResteasyDeployment}</li>
 *     <li>requires {@link ResteasyRegistration} to be registered as a bean before context is refreshed</li>
 *     <li>will not handle registering the above instances as beans</li>
 * </ul>
 *
 * <p>Original javadoc below:</p>
 * 
 * <p>
 ...
 * @version $Revision: 1 $
 */
public class CustomSpringBeanProcessor implements BeanFactoryPostProcessor, SmartApplicationListener
{
	...
	protected Set<String> registrations = new HashSet<String>();
	...
	protected class ResteasyBeanPostProcessor implements BeanPostProcessor
	{
		 ...
		 * </ol>
		 *
		 * @see CustomSpringBeanProcessor.postProcessBeanFactory
		 */
				...
				throws BeansException
		{
			if (providerNames.contains(beanName))
			{
				...
				getProviderFactory().registerProviderInstance(bean);
			}
		...
		{
			return getProviderFactory().getInjectorFactory().createPropertyInjector(clazz, getProviderFactory());
		}
	...
	public CustomSpringBeanProcessor(ResteasyProviderFactory providerFactory)
	{
	...
	public void onApplicationEvent(ApplicationEvent event)
	{
		...
		for (SpringResourceFactory resourceFactory : resourceFactories.values())
		{
			registry.removeRegistrations(resourceFactory.getScannableClass());
		}
...
//	  }
		for (SpringResourceFactory resourceFactory : resourceFactories.values())
		{
			registry.addResourceFactory(resourceFactory, resourceFactory.getContext());
		}
	}
